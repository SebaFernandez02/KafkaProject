...
public class SuscribersController {
    ...
    @GetMapping("/park")
    public ResponseEntity<List<SuscriberDto>> getByPark(@RequestParam String type) {
        try {
            List<SuscriberDto> suscriberDtos = suscriberService.getByPark(type);
            return new ResponseEntity<>(suscriberDtos, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
